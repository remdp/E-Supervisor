...
public class CustomProgressIndicator extends View {
    ...
    public CustomProgressIndicator(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public CustomProgressIndicator(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public CustomProgressIndicator(Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init(@NonNull Context context, @Nullable AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.CustomProgressIndicator);
            progressColor = a.getColor(R.styleable.CustomProgressIndicator_progressColor, progressColor);
            progressBackgroundColor = a.getColor(R.styleable.CustomProgressIndicator_progressBackgroundColor, progressBackgroundColor);
            progressStrokeWidth = a.getDimensionPixelSize(R.styleable.CustomProgressIndicator_progressStrokeWidth, progressStrokeWidth);
            progressBackgroundStrokeWidth = a.getDimensionPixelSize(R.styleable.CustomProgressIndicator_progressBackgroundStrokeWidth, progressStrokeWidth);
            textColor = a.getColor(R.styleable.CustomProgressIndicator_textColor, progressColor);
            textSize = a.getDimensionPixelSize(R.styleable.CustomProgressIndicator_textSize, textSize);

            shouldDrawDot = a.getBoolean(R.styleable.CustomProgressIndicator_drawDot, true);
            dotColor = a.getColor(R.styleable.CustomProgressIndicator_dotColor, progressColor);
            dotWidth = a.getDimensionPixelSize(R.styleable.CustomProgressIndicator_dotWidth, progressStrokeWidth);

            startAngle = a.getInt(R.styleable.CustomProgressIndicator_startAngle, DEFAULT_PROGRESS_START_ANGLE);
            if (startAngle < 0 || startAngle > 360) {
                startAngle = DEFAULT_PROGRESS_START_ANGLE;
            }

            isAnimationEnabled = a.getBoolean(R.styleable.CustomProgressIndicator_enableProgressAnimation, true);
            isFillBackgroundEnabled = a.getBoolean(R.styleable.CustomProgressIndicator_fillBackground, false);

            direction = a.getInt(R.styleable.CustomProgressIndicator_directionC, DIRECTION_COUNTERCLOCKWISE);

            int cap = a.getInt(R.styleable.CustomProgressIndicator_progressCapC, CAP_ROUND);
            progressStrokeCap = (cap == CAP_ROUND) ? Paint.Cap.ROUND : Paint.Cap.BUTT;

            String formattingPattern = a.getString(R.styleable.CustomProgressIndicator_formattingPattern);
            ...
            final int gradientType = a.getColor(R.styleable.CustomProgressIndicator_gradientTypeC, 0);
            if (gradientType != NO_GRADIENT) {
                final int gradientColorEnd = a.getColor(R.styleable.CustomProgressIndicator_gradientEndColor, -1);
    ...
    public void drawCircularProgress(Boolean isAnimationEnabled,
                                     Boolean drawDot,
                                     @ColorInt int colorPi,
                                     @Dimension int indicatorWidth,
                                     @IntRange(from = 0, to = 360) int startAngle,
                                     int currentProgress,
                                     int maxProgress,
                                     ProgressTextAdapter progressTextAdapter) {

        setAnimationEnabled(isAnimationEnabled);

        //setTextColor(colorPi);
        textPaint.setColor(colorPi);
        Rect textRect = new Rect();
        textPaint.getTextBounds(progressText, 0, progressText.length(), textRect);

        //setShouldDrawDot(drawDot);
        shouldDrawDot = drawDot;
        if (dotPaint.getStrokeWidth() > progressPaint.getStrokeWidth()) {
            requestLayout();
            //return;
        }
        ...
        //setDotColor(colorPi);
        dotPaint.setColor(colorPi);

      //  setDotWidthDp(indicatorWidth);
        dotPaint.setStrokeWidth(dp2px(indicatorWidth));
      ...
      //  setProgress(currentProgress, maxProgress);
        ...
        if (direction == DIRECTION_COUNTERCLOCKWISE) {
            finalAngle = -(currentProgress / maxProgress * 360);
        } else {
            finalAngle = currentProgress / maxProgress * 360;
        }
        ...
        maxProgressValue = maxProgress;
        progressValue = Math.min(currentProgress, maxProgress);
        ...
        //  setGradient(CustomProgressIndicator.SWEEP_GRADIENT, colorPi);
        ...
        gradient = new SweepGradient(cx, cy, new int[]{startColor, colorPi}, null);
        ...
        //setProgressTextAdapter(progressTextAdapter);
        if (progressTextAdapter != null) {
            this.progressTextAdapter = progressTextAdapter;
        } else {
            this.progressTextAdapter = new DefaultProgressTextAdapter();
...
//    public void resetCircularProgress(int indicatorWidth = 10) {
//
